...
export default function PostFlight({ mission, checklist }: { mission: MissionProfile; checklist: MissionChecklistReport }) {
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Missions', href: '/missions' },
        { title: mission.mission_number, href: `/missions/${mission.id}` },
        { title: 'Post-flight checklist', href: `/missions/${mission.id}/post-flight-checklist` },
    ];
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Post-flight checklist - ${mission.mission_number}`} />
            <form onSubmit={submit} className="flex flex-1 flex-col gap-6 p-4 sm:p-6">
                <PageHeader title="Post-flight checklist" description={`${mission.mission_number} / ${checklist.template?.version ?? 'No active template'}`} actions={<Button variant="outline" asChild><Link href={`/missions/${mission.id}`}>Back</Link></Button>} />

                {!checklist.template ? (
                    <section className="rounded-lg border bg-card p-4 text-card-foreground shadow-xs">
                        <p className="text-sm text-muted-foreground">No active post-flight checklist template is configured.</p>
                    </section>
                ) : (
                    <>
                        <section className="rounded-lg border bg-card p-4 text-card-foreground shadow-xs">
